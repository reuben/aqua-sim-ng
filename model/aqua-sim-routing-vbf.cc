...
bool
AquaSimPktHashTable::GetHash(AquaSimAddress senderAddr, unsigned int pk_num, vbf_neighborhood& ret)
{
  auto it = m_htable.find(senderAddr);
  if (it == m_htable.end()) {
    return false;
  }

  auto it2 = it->second.find(pk_num);
...
AquaSimPktHashTable::PutInHash(AquaSimAddress sAddr, unsigned int pkNum, Vector p)
{
  ...
  if (it != m_htable.end()) {
    ...
    auto it2 = inner.find(pkNum);
    if (it2 != inner.end()) {
      int m = it2->second.number;
      if (m < MAX_NEIGHBOR) {
        it2->second.number++;
        it2->second.neighbor[m].x = p.x;
        it2->second.neighbor[m].y = p.y;
        it2->second.neighbor[m].z = p.z;
      }
      return;
    }
  }

  vbf_neighborhood ptr;
  ptr.number = 1;
  ptr.neighbor[0].x = p.x;
  ptr.neighbor[0].y = p.y;
  ptr.neighbor[0].z = p.z;
  m_htable[sAddr][pkNum] = ptr;
}
...
AquaSimVBF::Recv(Ptr<Packet> packet, const Address &dest, uint16_t protocolNumber)
{
  ...
  packet->AddHeader(ash);
    ...
    vbf_neighborhood nb;
    bool found = PktTable.GetHash(vbh.GetSenderAddr(), vbh.GetPkNum(), nb);
...
AquaSimVBF::ConsiderNew(Ptr<Packet> pkt)
{
  ...
  pkt->AddHeader(ash);
    ...
    case INTEREST:
        ...
        found = PktTable.GetHash(vbh.GetSenderAddr(), vbh.GetPkNum(), nb);
    NS_LOG_INFO("ConsiderNew: INTEREST, hashptr #=" << nb.number);
        ...
        if (found) {
            ...
            if (num_neighbor!=1) {
                ...
                else //I need to calculate my delay time again
                {
                    ...
                    tp[0].x=nb.neighbor[i].x;
                    tp[0].y=nb.neighbor[i].y;
                    tp[0].z=nb.neighbor[i].z;
